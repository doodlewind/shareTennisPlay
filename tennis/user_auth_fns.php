...
function login($name,$passwd) {
	...
	$sql = "select * from user where name=(?) and passwd_sha=sha1(?)";
	$stmt = $conn->prepare($sql);
	$stmt->bind_param("ss",$name,$passwd);
	$stmt->execute();
	$res = $stmt->get_result();
	$row = $res->fetch_assoc();
	$_SESSION['valid_id_ustc'] = $row['id_ustc'];//在register_verify.php中有同一句	
	if (!$stmt) {
		throw new Exception('无法连接到数据库，<a href="login.php"data-ajax="false">返回</a>');	
	}
	if ($row) {
		return true;
	}else{
		throw new Exception('用户名或密码错误，<a href="login.php"data-ajax="false">返回</a>');
	}
}
function init_member() {
	...
	if ($name && $passwd) {
		try {
			login($name,$passwd);//in user_auth_fns.php
		}
		catch(Exception $e){
			set_html_header();
...
function check_valid_id() {
	...
	if(isset($_SESSION['valid_id_ustc'])) {
		...
		echo_event("错误提示","你尚未登陆，请<a href=\"login.php\"data-ajax=\"false\">重试</a>");
		exit();
	}
}
/*
